...
+ #### 資料檔
	+ 紀錄資料，如音訊檔路徑和對應的文字輸出內容等依據具需求增添的欄位資訊。訓練模型時，載入資料不代表一定會用上所有欄位，屆時會有很多相關的函式或方法協助使用者過濾篩選。
* * *
...
dataset
    ...
    └─ train
           ...
           └─ 訓練集音訊檔_3.wav
```
> 註: 同檔名的 metadata.csv 裡，盡管格式相同，但描述的資料不同
***
...
dataset
    ...
    └─ train
           ...
           └─ 訓練集音訊檔_3.wav
```
+ dataset: 資料夾，這個資料集名稱，名稱沒有限制，至少要看的出來找的到。
+ train & test: 資料夾，名稱**至少分別包含關鍵字 "train"、"test"**，也可以是以此名稱延伸的檔名，載入時會自動辨別，否則會讀不到。淪為只讀取訓練集(這時就不會有命名要求)。

> 註: 有關音訊檔分配的部分，示範程式 "assort_data.py" 會協助隨機分配，相關使用說明後面會提及。
